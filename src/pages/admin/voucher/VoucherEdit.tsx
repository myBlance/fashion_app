              ...
              <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ flex: 1, minWidth: '300px' }}>
                  <TradingInputWrapper color="#1976d2" label="Mã Voucher (Code)">
                    <TextInput source="code" validate={required()} fullWidth label="" />
                  </TradingInputWrapper>
                </Box>
              </Box>
              <Box sx={{ mt: 3 }}>
                <TradingInputWrapper color="#0288d1" label="Tên chương trình">
                  <TextInput source="name" validate={required()} fullWidth label="" />
                </TradingInputWrapper>
              </Box>
            </CardContent>
          </Card>
          ...
          <Box sx={{ display: 'flex', gap: 3 }}>
            <Box sx={{ flex: 2 }}>
              <TradingInputWrapper color="#607d8b" label="Giới hạn sử dụng">
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <NumberInput source="maxUses" min={0} validate={[minValue(0)]} label="Tổng lượt dùng" sx={{ flex: 1 }} />
                  <NumberInput source="maxUsesPerUser" min={0} validate={[minValue(0)]} label="Lượt dùng/Khách" sx={{ flex: 1 }} />
                </Box>
              </TradingInputWrapper>
            </Box>
            <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 0 }}>
              <Card elevation={0} sx={{ border: '1px dashed #bdbdbd', borderRadius: 2, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#fafafa' }}>
                <BooleanInput source="isActive" label="Kích hoạt ngay" sx={{ '& .MuiFormControlLabel-label': { fontWeight: 'bold', color: '#2e7d32' } }} />
              </Card>
            </Box>
          </Box>

          <TradingInputWrapper color="#757575" label="Ghi chú nội bộ">
            <TextInput source="description" fullWidth multiline rows={3} label="" />
          </TradingInputWrapper>
